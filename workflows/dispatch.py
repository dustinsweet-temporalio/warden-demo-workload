    ...
    async def run(self, state: DispatchState) -> None:
        ...
        while True:
            if self._degr.inflate_history:
                ...
                # Wake on work, on a degradation flip, or when history is large
                # enough that we must continue-as-new (so a reversed S2 can shed
                # its accumulated history even with no pending work).
            ...
            if not self._degr.suspend_can and (
                ...
                workflow.continue_as_new(args=[self._carry()])
